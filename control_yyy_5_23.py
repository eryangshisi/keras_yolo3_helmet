...
#三次更新
#增加了新表Admin的写入add_Admin(self,Admin,Akey,Aidentity)和读取def return_Admin(self)函数
import pymysql
...
class SQL_Y():
    ...
    def return_DateInfo(self, DtimeOfdate):
        ...
        sql='select * from DateInfo where DtimeOfdate='+'\''+DtimeOfdate+'\';';
        cur.execute(sql)
        return cur.fetchall()
    ...
    def add_Admin(self,Admin,Akey,Aidentity):
        if Aidentity==1 or Aidentity==0:
            cur=self.key.cursor()
            sql = 'insert into Admin(Admin,Akey,Aidentity)value(%s,%s,%s)'
            data = [(Admin,Akey,Aidentity)]
            cur.executemany ( sql , data )
            self.key.commit ()
        else:
            print("身份信息错误")
    ...
    def return_Admin(self, num):
        cur = self.key.cursor()
        sql = 'select * from Admin where Admin=%s;'
        cur.execute(sql, [num])
        results = cur.fetchone()
        passwords = results[1]
        return passwords

    def have_Admin(self, num):
        cur = self.key.cursor()
        sql = 'select * from Admin where Admin=%s;'
        have = cur.execute(sql, [num])
        return have
...
# if __name__ == '__main__':
    ...
    ## key.close() ##
    # db = SQL_Y()
    # print(db.return_Admin("20200303003"))
    # db.add_worker("小蒲","20200303008","女","18","2020-03-04","挖煤部","E:\\HafetyHelmet\\idcard\\20200303008.jpg","0","")
    # ====[√]测试print===
    # db.print_data_info()
    # ====[√]测试更新犯罪数据=====
    ...
    # db.add_Admin("20200303002","002",2)
    # db.add_Admin ( "20200303002" , "002" , 1 )
    # results=db.return_Admin()
    # for row in results :
